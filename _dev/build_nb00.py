"""Build notebook 00 directly as JSON."""
import json
...
SUMMARY = (
    ...
    '- Bekaert, G. & Harvey, C.R. (2002). Research in emerging markets finance: looking to the future. *Emerging Markets Review*, 3(4), 429-448. https://doi.org/10.1016/S1566-0141(02)00045-6 — [free PDF](https://business.columbia.edu/sites/default/files-efs/pubfiles/1963/1963B.pdf)\n'
)

cells = [
    md('# 00 - Uvod i metodologija: Financijske mreze na trzistu kapitala\n\n'
    ...
    COLAB,
    md('## 1. Zasto mrezna analiza financijskih trzista?\n\n'
       'Klasicni modeli (Markowitz, CAPM) sazimaju odnose medu dionicama u jednu korelacijsku matricu. '
       'Mrezna analiza ide dalje: **struktura** te matrice nosi vlastitu informaciju o stanju trzista.\n\n'
       '| Pitanje | Mjera |\n|---------|-------|\n'
    ...
    md('## 2. Metodoloski pipeline: od prinosa do mreze\n\n'
       ...
       'Zasto ne standardni indeks? Kada bi se koristio puni indeks, regresija '
       '$r_{it} = \\alpha_i + \\beta_i \\cdot I_t + \\varepsilon_{it}$ bila bi cirkularna: '
       '$I_t$ je ponderirana suma svih $r_{jt}$ ukljucujuci i $r_{it}$ sam. '
       'To napuhuje betu i pristrano iskrivljuje rezidualne korelacije (Roll, 1977; Cohen, 2025). '
       'EW-LOO eliminira taj problem konstrukcijom.\n\n'
       '> **Napomena:** U istrazivanju su paralelno koristena oba faktora - EW-LOO i puni CROBEX indeks '
       '(oznake `/loo` i `/idx` u tablicama rezultata). Usporedba pokazuje koliko je izbor trzisnog '
       'faktora utjece na nalaize - a to je samo po sebi informativno.\n\n'
       ...
       'Za svaki par testiramo $H_0: \\rho_{ij}^{\\text{parc}} = 0$. '
       'Brid postoji ako je $p_{ij} < \\alpha$.\n\n'
       ...
       '> Negativna parcijalna znaci: kada dionica $i$ nadmasuje trziste (poz. rezidual), '
       'dionica $j$ ga podmasuje (neg. rezidual) - i obrnuto.  \n'
       '> To su **relativno suprotna kretanja u idiosinkratskim komponentama** - nakon uklanjanja zajednickog faktora.\n'),
    md('## 3. Podaci: CROBEX i Zagrebacka burza\n\n'
...
out = Path("notebooks/00_uvod_i_metodologija.ipynb")
